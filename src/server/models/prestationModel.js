...
export default class Prestation{
    ...
    static async getAllPrestations(id){
        var data = [] ;
        try{
            data = await dbQuery("SELECT * from ab_prestations WHERE prestation_bank_id=(?)",[id]) ;
        }catch(err){
            throw new ModelError(errorMessages.serverError,500) ;
        }
    ...
    static async getPrestationById(id){
        ...
        try {
            var data = await dbQuery("SELECT * FROM ab_prestations WHERE id_prestation=(?)",[id]) ;
        } catch(err){
            throw new ModelError(errorMessages.serverError,500) ;
        } 
    ...
    static async getPrestationByName(id_bank,name, type){
        try {
            var data = await dbQuery("SELECT * FROM ab_prestations WHERE prestation_bank_id=(?) AND prestation_name=(?) AND prestation_type=(?)", [id_bank,name,type]) ;
