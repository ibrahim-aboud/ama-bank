...
function BankInfoFormADD() {
  ...
  async function sumbitHandler(event) {
    ...
    try {
      ...
      const response = await axios.post(process.env.NEXT_PUBLIC_API_URL + "/banks", {bank: bank});
        
      const data = response.data;

      if (selectedFile1) { // logo
        const fileExtension = selectedFile1.name.split(".").pop();
        const file = new File([selectedFile1], `${data.id}.${fileExtension}`);
      ...
      if (selectedFile2) { // image
        ...
        const file = new File([selectedFile2], `${data.id}.${fileExtension}`);
