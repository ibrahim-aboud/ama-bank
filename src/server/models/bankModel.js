...
export default class Bank {
  ...
  static async updateBank(bank) {
    try{
      ...
      var row = await dbQuery("SELECT * FROM ab_banks WHERE id_bank=(?)", [bank.id])
    ...
    try{
      ...
      "INSERT INTO ab_banks VALUES((?), (?), (?), (?), (?), (?), (?), '"+year+"-"+month+"-"+day+"', 'MODIFIED')", 
  ...
  static async deleteBank(id) {
    ...
    try{
      ...
      var rowPres = await dbQuery("SELECT * FROM ab_prestations WHERE prestation_bank_id=(?)", [id])
    ...
    try{
      ...
      var rowAgen = await dbQuery("SELECT * FROM ab_agencies WHERE agency_bank_id=(?)", [id])
    ...
    try{
      ...
      var rowDg = await dbQuery("SELECT * FROM ab_dgs WHERE dg_bank_id=(?)", [id])
    ...
    try{
      ...
      "INSERT INTO ab_banks VALUES((?), (?), (?), (?), (?), (?), (?), '"+year+"-"+month+"-"+day+"', 'DELETED')", 
      [null, row[0].id_bank, row[0].bank_name, row[0].bank_description, row[0].bank_visits_count, row[0].bank_website_link, 
      row[0].bank_update_date]
          )
    ...
    try{
      ...
      var dataToArchiveAgencies = await dbQueryArchive(
        "INSERT INTO ab_agencies VALUES((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), '"+year+"-"+month+"-"+day+"', 'INSERTED')", 
    ...
    try{
      ...
      for(let i in rowPres){
        var dataToInsertPres = await dbQueryArchive(
          "INSERT INTO ab_prestations VALUES((?), (?), (?), (?), (?), (?), (?), (?), (?), '"+year+"-"+month+"-"+day+"', 'INSERTED')", 
    ...
    try{
      ...
      if(rowDg.length != 0){
        var dataToArchiveDgs = await dbQueryArchive(
            "INSERT INTO ab_dgs VALUES((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), '"+year+"-"+month+"-"+day+"', 'INSERTED')", 
